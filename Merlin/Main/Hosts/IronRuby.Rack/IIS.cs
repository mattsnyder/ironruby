...
namespace IronRuby.Rack.Handler {
    public class IIS {
        ...
        public void Handle(Request request, Response response) {
            ...
            if (((MutableString)GetEnv(env, "CONTENT_TYPE")).Length == 0) {
                RemoveEnv(env, "CONTENT_TYPE");
            }
        ...
        private void SetEnv(Hash env, string key, string value) {
            env[MutableString.CreateAscii(key)] = MutableString.CreateAscii(value);
        }

        private void SetEnv(Hash env, string key, object value) {
            env[MutableString.CreateAscii(key)] = value;
        }

        private void RemoveEnv(Hash env, string key) {
            env.Remove(MutableString.CreateAscii(key));
        }

        private object GetEnv(Hash env, string key) {
            return env[MutableString.CreateAscii(key)];
        }
    }
}
